    ...
    public class RaceSelectionTab : TFWindowTab
    {
        ...
        public override void Draw(Rect inRect, Pawn pawn, AmphiShifter shifter)
        {
            ...
            for (int i = 0; i < shifter.knownSpecies.Count; i++)
            {
                ...
                Widgets.DrawTextureFitted(new Rect(new Vector2(inRect.position.x + 10,boxHeight*length+20),new Vector2(boxHeight,boxHeight)),tex,1);
                for(int a =0; a < storedRaces.Count; a++)
                {
                    StoredRace race = storedRaces[a];
                    if (a == 0)
                    {
                        Widgets.Label(new Rect(new Vector2(inRect.position.x + 90, boxHeight * length + 20), new Vector2(inRect.width - 90, boxHeight)), (race.XenotypeDef != null ? race.XenotypeDef.defName : race.ThingDef.defName));
                        continue;
                    }
                    if (a > 0)
                    {
                        ...
                        Widgets.DrawTextureFitted(new Rect(new Vector2(inRect.position.x + 10, boxHeight * length + 20), new Vector2(boxHeight, boxHeight)), icon, 1);
                       
                    }
                    Widgets.Label(new Rect(new Vector2(inRect.position.x + 90, boxHeight * length + 20 + boxHeight), new Vector2(inRect.width - 90 - boxHeight, boxHeight)), (race.XenotypeDef != null ? race.XenotypeDef.defName : race.ThingDef.defName));
                    length++;
                }
                length++;
